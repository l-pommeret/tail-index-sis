...
sample sizes are effectively small. One of the findings below is that this
count, which is the right one for a \emph{local} profile, is not the one
that governs a screening statistic built by integrating such profiles.
...
\[
 O_{\Pp}\!\left(\sqrt{\frac{\log(pn)}{n\alpha}}\right)+o(\Delta_{\min}),
\]
...
$\log(pn)=o(n\alpha\Delta_{\min}^2)$. The bandwidth is absent from this
rate. Each local profile does fluctuate at scale $(n\alpha h)^{-1/2}$,
but the score averages $O(n)$ of them, and colouring the rank grid into
classes of pairwise disjoint windows shows that the effective sample size
of the average is the tail count $n\alpha$: high dimension is affordable
exactly when the tail count dominates the logarithm of the

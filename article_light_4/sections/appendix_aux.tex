...
\[
 \frac{n\alpha h}{4}\;\le\;K_n\;\le\;2n\alpha h .
\]
\end{lemma}
...
$\alpha m_n^-\ge2$ (since $\lfloor x\rfloor\ge x/2$ for $x\ge2$).  The
upper bound follows from $m_n^-\le2h(n+1)\le2hn+2$ and
$K_n\le\alpha m_n^-$.
\end{proof}
...
\subsection{The bandwidth-free integrated rate}\label{app:colour}

Proposition~\ref{thm:app-score} bounds the integrated score by the
supremum of the profile errors.  That step is what puts $n\alpha h$ in
the rate, and it is lossy: $\widehat\Psi_j$ is an average of $L_n\asymp n$
local statistics, and the fluctuation of an average is not the largest
of the fluctuations.  This subsection replaces the step.

Throughout, $\delta_n=1/(n+1)$, $u_r=r\delta_n$, and
$\mathcal R_n=\{r\le n:u_r\in\I\}$ is an interval of consecutive
integers with $L_n=|\mathcal R_n|$.  Write $\mathcal W_{jr}$ for the
window at $u_r$, $m_r$ for its size and $k_r=\lfloor\alpha m_r\rfloor$,
so $k_r\ge K_n$.  Let $L_{jr}$ denote the R\'enyi mean of
Lemma~\ref{lem:app-hill} attached to $\mathcal W_{jr}$.  Set
\begin{equation}
 q_n^{\rm sep}=\lfloor2h(n+1)\rfloor+1,
 \qquad
 \chi_n=\min(q_n^{\rm sep},L_n),
 \qquad
 \mathfrak n_n=\frac{L_nK_n}{\chi_n}.
 \label{eq:effn}
\end{equation}

\begin{lemma}[Signed local decomposition]\label{lem:app-signed}
On the event of Lemma~\ref{lem:app-hill}, simultaneously for all $j\le p$
and $r\in\mathcal R_n$,
\[
 \widehat\xi_j(u_r)-\xi_j(u_r)
 =\xi_j(u_r)\{L_{jr}-1\}+R_{jr},
 \qquad
 |R_{jr}|\le2\omega_n(h+\eta+\delta_n,\tau)+b_n(3\alpha)L_{jr}.
\]
\end{lemma}

\begin{proof}
The proof of Lemma~\ref{lem:app-hill} produces this identity before the
triangle inequality is applied; \eqref{eq:A3app} is its absolute-value
consequence.  Retaining the sign of $L_{jr}-1$ is the only change.
\end{proof}

\begin{lemma}[Colouring the rank grid]\label{lem:app-colour}
Colour $r\in\mathcal R_n$ by its residue modulo $q_n^{\rm sep}$ when
$q_n^{\rm sep}\le L_n$, and by singletons otherwise; this produces
$\chi_n$ nonempty classes.  Then, for every coordinate $j$, two distinct
windows whose centres lie in the same class are disjoint, and
conditionally on $\bm U_j$ the variables $\{L_{jr}\}$ indexed by one
class are independent.
\end{lemma}

\begin{proof}
If $q_n^{\rm sep}\le L_n$ and $r\ne s$ share a colour then
$|r-s|\ge q_n^{\rm sep}$, so $|u_r-u_s|\ge q_n^{\rm sep}\delta_n>2h$.
A row belonging to both windows would give
$|u_r-u_s|\le|u_r-\widehat U_{ij}|+|\widehat U_{ij}-u_s|\le2h$, a
contradiction; the singleton case is trivial.  Disjoint windows are
deterministic given $\bm U_j$ and involve disjoint sets of rows, so by
Lemma~\ref{lem:app-pit} their normalized top spacings are built from
disjoint collections of i.i.d.\ uniforms, whence independence.
\end{proof}

\begin{lemma}[Coloured spacing inequality]\label{lem:app-colourbern}
Fix $j$, condition on $\bm U_j$, and let $a_r\in[-1,1]$ be deterministic.
Then for every $t>0$
\[
 \Pp\Bigl\{\Bigl|\frac1{L_n}\sum_{r\in\mathcal R_n}a_r(L_{jr}-1)\Bigr|>t
 \;\Big|\;\bm U_j\Bigr\}
 \le2\exp\Bigl\{-\frac{\mathfrak n_n}{4}(t^2\wedge t)\Bigr\}.
\]
\end{lemma}

\begin{proof}
Write $S=L_n^{-1}\sum_ra_r(L_{jr}-1)$ and $S_c$ for the partial sum over
the colour class $c$, so $S=\sum_{c}S_c$.  If $L_k$ is a mean of $k$
i.i.d.\ standard exponentials then, for $|\lambda|<k$,
$\log\E e^{\lambda(L_k-1)}\le\lambda^2/\{2k(1-|\lambda|/k)\}$: for
$0\le\lambda<k$ and $x=\lambda/k$ the left side equals
$k\{-x-\log(1-x)\}\le kx^2/\{2(1-x)\}$, and the case $\lambda<0$ follows
from $x-\log(1+x)\le x^2/2$.

By H\"older's inequality applied to the $\chi_n$ colour classes,
\[
 \log\E(e^{\theta S}\mid\bm U_j)
 \le\frac1{\chi_n}\sum_{c=1}^{\chi_n}
   \log\E(e^{\chi_n\theta S_c}\mid\bm U_j),
\]
and within a class the summands are independent by
Lemma~\ref{lem:app-colour}, so the displayed mgf bound applies term by
term.  With $|a_r|\le1$ and $k_r\ge K_n$ this yields, for
$\chi_n|\theta|<L_nK_n$,
\[
 \log\E(e^{\theta S}\mid\bm U_j)
 \le\frac{\theta^2v_n}{2(1-c_n|\theta|)},
 \qquad
 v_n=\frac{\chi_n}{L_n^2}\sum_{r\in\mathcal R_n}\frac1{k_r}
 \le\frac1{\mathfrak n_n},
 \qquad
 c_n=\frac{\chi_n}{L_nK_n}=\frac1{\mathfrak n_n}.
\]
Thus $S$ is sub-gamma with variance factor and scale both at most
$1/\mathfrak n_n$, and the stated two-sided bound is the corresponding
Bernstein estimate.
\end{proof}

\begin{proposition}[Integrated-score bound at scale $n\alpha$]
\label{thm:app-score-na}
Under the conditions of Proposition~\ref{thm:app-score}, for every
$\eta>0$, $0<t\le1$, $0<\tau<1/2$,
\[
 \Pp\Bigl\{\max_{j\le p}|\widehat\Psi_j-\Psi_j|
 >\widetilde B_n(\eta,t,\tau)+q_n\Bigr\}
 \le2pe^{-2n\eta^2}+2pn\tau+pS_ne^{-(K_n+1)/4}
   +4pe^{-\mathfrak n_nt^2/4},
\]
where
$\widetilde B_n(\eta,t,\tau)
 =2\omega_n(h+\eta+\delta_n,\tau)+(\Gamma+b_n(3\alpha))\,t
 +b_n(3\alpha)$.
\end{proposition}

\begin{proof}
Average the identity of Lemma~\ref{lem:app-signed} over $r\in\mathcal R_n$
\emph{before} taking absolute values.  Writing
$A_j=L_n^{-1}\sum_r\xi_j(u_r)\{L_{jr}-1\}/\Gamma$ and
$C_j=L_n^{-1}\sum_r\{L_{jr}-1\}$, both of the form treated by
Lemma~\ref{lem:app-colourbern} since $\xi_j/\Gamma\in[0,1]$, one gets
pathwise on the event of Lemma~\ref{lem:app-hill}
\[
 \Bigl|\frac1{L_n}\sum_{r}\widehat\xi_j(u_r)-\frac1{L_n}\sum_r\xi_j(u_r)\Bigr|
 \le2\omega_n(h+\eta+\delta_n,\tau)+\Gamma|A_j|
   +b_n(3\alpha)\{1+|C_j|\},
\]
and Lemma~\ref{lem:app-quadrature} bounds the passage from the grid
average to $\Psi_j$ by $q_n$.  Lemma~\ref{lem:app-colourbern} applied
twice, with a union bound over $j\le p$, contributes
$4pe^{-\mathfrak n_nt^2/4}$; the remaining three terms are the events of
Proposition~\ref{thm:app-score} that are not used per window state.

Each colour class is centred at its own average of $\xi_j$ over the
sparse grid it indexes, and those $\chi_n$ sparse averages recombine
exactly into the fine-grid average $L_n^{-1}\sum_r\xi_j(u_r)$.  No
modulus of continuity of $\xi_j$ at scale $h$ is therefore incurred,
and the bias terms are those of Proposition~\ref{thm:app-score}.
\end{proof}

\begin{remark}[Where the bandwidth goes]\label{rem:app-hgone}
By Lemma~\ref{lem:app-kn}, $K_n\ge n\alpha h/4$, while
$\chi_n\le2h(n+1)+1\le Cnh$ and $L_n\ge c_\varepsilon n$; hence
\[
 \mathfrak n_n=\frac{L_nK_n}{\chi_n}\ge c'_\varepsilon\,n\alpha .
\]
The factor $h$ in $K_n$ is cancelled by the factor $h$ in the number of
colours.  It survives elsewhere: in $K_n\gg\log(pn)$, which is what
places every local threshold inside the tail region, and in the
rank-displacement condition $\log(pn)=o(nh^2)$.
\end{remark}
...
(Proposition~\ref{prop:geometry}), hence

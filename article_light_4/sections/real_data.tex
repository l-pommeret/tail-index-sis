...
$\alpha=n^{-0.35}\approx0.070$ and $h=n^{-0.15}/2\approx0.160$, so the
sample contains $n\alpha\approx140$ upper-tail observations in total and
about $2n\alpha h\approx45$ within a typical interior rank window. The
...
tail modelling with $99$ covariates is poorly supported by roughly $140$
...
$(a^\star,b^\star)=(0.35,0.15)$ is reported alongside. The two
...
selected-pair ranks $18$ and $22$), so their rankings are markedly more

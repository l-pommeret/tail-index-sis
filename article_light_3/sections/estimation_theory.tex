...
fluctuations. The windows overlap, so the local errors are dependent ---
yet two windows whose centres are separated by more than $2h(n+1)$ rank
positions are \emph{disjoint}. Colouring the grid by residue modulo
$q_n=\lfloor2h(n+1)\rfloor+1$ therefore splits it into $\chi_n\asymp nh$
classes inside which the local statistics are conditionally independent,
and averaging the \emph{signed} R\'enyi-spacing fluctuations within each
class before bounding them gives an effective sample size
\[
 \mathfrak n_n=\frac{L_nK_n}{\chi_n}
 \asymp\frac{n\cdot n\alpha h}{nh}
 \asymp n\alpha .
\]
The two factors of $h$ cancel: the colouring costs $\chi_n\asymp nh$,
and each of the $L_n\asymp n$ terms contributes $1/k_r\asymp1/(n\alpha h)$.
The quantity $n\alpha$ --- the number of observations in the tail, not
the number in the tail \emph{of one window} --- is the effective sample
size of the integrated score.

Two features of the argument matter and are easy to get wrong. The
average must be taken of the signed fluctuations, not of the profilewise
absolute bounds: taking absolute values first destroys exactly the
cancellation being exploited. And each colour class must be centred at
its own sparse-grid population average; those sparse-grid averages
recombine exactly into the original fine-grid average, so no additional
smoothness assumption of the form $\omega_\xi(h)=o(\Delta_{\min})$ is
incurred.

The $o(\Delta_{\min})$ term is unchanged: it collects the tail bias (E2),
...
slowly varying remainder bounded through (E2); (iv) the signed
fluctuations are averaged over the rank grid coloured by residue modulo
$\lfloor2h(n+1)\rfloor+1$, within which windows are disjoint and hence
conditionally independent, and a sub-gamma bound applied per colour
class before recombination yields the effective sample size
$\mathfrak n_n\asymp n\alpha$; (v) a union bound over
the $p(4n+1)$ states yields the $\log(pn)$ factor; (vi) uniform score

...
of Sure-20 is largest at $(0.30,0.15)$, at $0.780$, with
$(0.35,0.15)$ next at $0.7625$; the four remaining cells of the block
lie between $0.718$ and $0.756$. We adopt
\[
 (a^\star,b^\star)=(0.35,\ 0.15)
\]
rather than the raw argmax, for two reasons. First, the gap between the
two leading cells, $0.018$ on the four-model mean, is of the order of
one paired standard error, so the argmax is not separated from its
neighbour by the evidence available here; selecting the argmax of the
same $200$ replications that are displayed in Figure~\ref{fig:tuning}
would be the more optimistic of the two choices. Second, of the two,
$a=0.35$ is the one further from the small-$a$ edge at which the A-family
models collapse, and that collapse is a bias phenomenon: a larger $a$
means a smaller tail fraction and therefore a local Hill statistic that
reaches less far into the region where $\ell_1$ deforms the projected
distributions. The cost of the choice is borne by A2, the weakest
signal, whose Sure-20 falls from $.555$ to $.475$; the other three
models are at least as good at $(0.35,0.15)$, with per-model Sure-20
$.960$, $.475$, $.885$ and $.730$.

...
consensus block at aggregated ranks $7$--$8$. For these variables the
body and the tail of the crime distribution point at the same
covariates, and every method agrees.
...
The minimum aggregation also exhibits its known failure mode, and the
per-setting ranks make it visible: \texttt{pctLargHous} (aggregated
rank $9$) and \texttt{pctHousWOplumb} (rank $10$) owe their positions
to a single favourable setting each (per-setting ranks $6$--$74$ and
$6$--$55$; selected-pair ranks $46$ and $36$). With $95$ competitors, a
coordinate promoted by one setting out of nine warrants no
interpretation, and we draw none. The consensus block and the
tail-specific candidates, by contrast, hold their positions across the
whole block.

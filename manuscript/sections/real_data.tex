...
dominate every competing ranking as well. The poverty rate
(\texttt{pctPoverty}) is first, with per-setting ranks between $1$ and
$6$ across the nine settings; the two-parent household variables
\texttt{pct2Par} and \texttt{pctKids2Par} are third and fourth (ranks
$1$--$5$), and both are also leading variables for the quantile screen
at every level (\texttt{pctKids2Par} is first at all three $\tau$)
and for Yoshida--Umezu (ranks $3$--$7$). The related
...
(\texttt{pctMaleNevMar}, aggregated rank $2$, per-setting ranks
...
\texttt{pctLargHous} (aggregated rank $9$) and \texttt{pctHousWOplumb}
(rank $10$) illustrate the resulting behaviour: their positions rest on
one favourable setting each (per-setting ranks $6$--$74$ and $6$--$55$;
selected-pair ranks $46$ and $36$), so their rankings are markedly more
...
and confounding among the $99$ predictors is certain. The ranks
reported here carry no inferential calibration; Section~\ref{sec:llm}
shows how a permutation threshold converts the screen into a test and
what such a test refuses to declare. A second caution comes from the
boundary experiment of Section~\ref{sec:llm}: at
$2n\alpha h\approx65$ --- the regime of this application, imposed by
$n=1{,}993$ --- the tail screen ranked a pure scale cluster first on
the language data, an artefact that disappeared at
$2n\alpha h\approx232$. The tail-specific candidates of this section
(\texttt{pctUsePubTrans}, \texttt{pctWfarm}, \texttt{pctMaleNevMar})
could in principle be scale drivers promoted by the same second-order
mechanism, and nothing in these data rules that out; they are
candidates for a second-stage analysis under that explicit caveat, and
grafting a known scale cluster onto this design, as in
Section~\ref{sec:llm}, would measure the contamination directly.

...
\item[(E2)] \emph{Uniform tail regularity.} By
Proposition~\ref{prop:projection} and inversion
(Lemma~\ref{lem:app-quantile-rv}), $Q_j(\cdot,u)$ is regularly varying
with index $\xi_j(u)$, so it admits the representation
$Q_j(s,u)=s^{\xi_j(u)}\ell_j(s,u)$ with $\ell_j(\cdot,u)$ slowly
varying; we require
\[
...
coordinate; (iii) the $pn$ conditional probability-integral transforms
are truncated to $[\tau_n,1-\tau_n]$ at probability cost $2pn\tau_n$,
which is what fixes $\tau_n=(pn)^{-2}$ in (E3); (iv) conditionally on a
covariate column, the R\'enyi representation turns each local Hill
statistic into a mean of i.i.d.\ exponentials built from about
$n\alpha h$ extremes, plus a slowly varying remainder bounded through
(E2); (v) a union bound over the $p(4n+1)$ states yields the
$\log(pn)$ factor; (vi) a deterministic quadrature step converts
profile accuracy into score accuracy at cost
$\omega_\xi(1/n)+O(1/n)$; (vii) uniform score accuracy below
$\Delta_{\min}/2$ gives deterministic separation of active from
inactive scores. \citet[Theorem~3]{gardesPodgorny2025}
...
$\log p=o(n^{1-a-b}\Delta_{\min}^2)$ are admissible --- a range of $p$
growing as $\exp\{o(n^{1-a-b}\Delta_{\min}^2)\}$, nearly exponential in
a power of $n$ for fixed gaps; for $b>a$ the rank-displacement condition

...
$\widehat\A_d$ collects the $d$ smallest scores, ties broken by
coordinate index.
...
\begin{remark}[Scale nuisances enter through (E3)]\label{rem:scale}
A covariate that shifts the conditional scale of $Y$ without touching
the index makes the projected quantiles move multiplicatively with $u$
at every finite level: $\omega_n(C_0h,\tau_n)$ is then of the order of
the log-scale amplitude, a constant, and (E3) fails at any fixed
amplitude. For the scale factor of model B1 in
Section~\ref{sec:simulation}, $\exp\{0.5(\Phi(F)-0.5)\}$, the log-scale
amplitude is bounded by $0.5$, larger than the population gap
$\Delta_{\min}=0.186$; the condition is recovered only as the local
extreme count grows and the Hill statistic escapes the finite-level
region where the scale acts. Scale robustness is therefore an
asymptotic property of the screen, with a finite-sample onset that
Section~\ref{sec:comparison} measures.
\end{remark}

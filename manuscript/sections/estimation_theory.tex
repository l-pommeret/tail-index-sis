...
\end{equation}
with $\widehat\Psi_j=+\infty$ when no grid point is usable, so that
underpopulation can never favour a coordinate. Because the window counts
$M_j(u)$ depend on the ranks only through the grid position, $G_j^{\rm
val}$ is in fact the same set for every $j$; under the conditions of
Theorem~\ref{thm:score} every grid point is usable and $G_j^{\rm
val}=G$. The screened set $\widehat\A_d$ collects the $d$ smallest
scores, ties broken by coordinate index.
...
their fixed-dimension truncation. Their density conditions (H.1)--(H.2)
...
These conditions are stated on the projected quantiles, not derived from
(C1)--(C2): projection can generate genuinely new slow variation. If
$W\sim U(0,1)$ and $\Pp(Y>y\mid W)=y^{-1/(1+W)}$ exactly, then
$\Pp(Y>y)\sim4y^{-1/2}/\log y$, so even exact Pareto conditionals produce a
logarithmic factor after projection. Appendix~\ref{app:examples} establishes
(E1)--(E3) for finite mixtures with a spectral gap, where the left side
of (E2) is $O(\alpha^{\rho})$, and sketches the same for a dependent
plateau-maximizer class where it is
$O(\{\log(1/\alpha)\}^{-\{1+(s-1)/a_0\}})$, $a_0$ being the flatness of
the plateau boundary; the least favourable case $s=2$ gives exponent
$1+1/a_0$.
...
$a_n=O(\sqrt{\log(pn)/(n\alpha h)})$ and $b_n=o(\Delta_{\min})$ with
\[
 \Pp\Bigl(\max_{j\le p}\,|\widehat\Psi_j-\Psi_j|\le a_n+b_n\Bigr)
 \longrightarrow1;
\]
in shorthand,
...
The proof passes through a uniform bound on the whole profile
$\max_j\sup_{u\in\I}|\widehat\xi_j(u)-\xi_j(u)|$, stated with explicit
finite-sample constants as Theorem~\ref{thm:app-rank-profile} in the
appendix; Theorems~\ref{thm:score} and~\ref{thm:sis} are its asymptotic
readings. The argument has four ingredients. A single
Dvoretzky--Kiefer--Wolfowitz event per coordinate controls all rank
displacements at once, contributing the $\log(pn)=o(nh^2)$ requirement. Rank
windows have deterministic sizes, at least
$\lfloor\{h+\min(h,\varepsilon)\}(n+1)\rfloor-1$ observations each, and, as
$u$ moves, pass through at most $4n+1$ membership states per coordinate.
Conditionally on a covariate column, the probability-integral transforms of
the responses are i.i.d.\ uniform, and the R\'enyi representation turns
each local Hill statistic into a mean of i.i.d.\ exponentials plus a slowly
varying remainder bounded through (E2). A union bound over the $p(4n+1)$
states then gives the $\log(pn)$ factor; no independence across
overlapping windows or dependent coordinates is used. Finally, score
separation is deterministic: when every score error is below
$\Delta_{\min}/2$, every active score lies below $\gmax-\Delta_{\min}/2$
and every inactive score above it. \citet[Theorem~3]{gardesPodgorny2025}
establishes the corresponding uniform consistency in fixed dimension over a
compact class of projection matrices; the present setting replaces their
covering-number argument and density assumptions by the rank construction
above, and their result does not imply the growing-$p$ bound needed here.
...
$\log(pn)=o(nh^2)=o(n^{1-2b})$ binds first. These ranges treat (E3) as
given: in the verified mixture class of
Proposition~\ref{prop:app-mixture} the spatial modulus adds the
constraint $\log(pn)=o(n^{b})$, giving the range
$\log(pn)=o(n^{\min(b,1-a-b)})$ stated there.

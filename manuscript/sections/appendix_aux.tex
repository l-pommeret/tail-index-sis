...
Under (C1), the map $u\mapsto\max_v\gamma(u,v)$ is continuous on
$[0,1]$, with modulus bounded by the modulus of continuity of
$\gamma$; under (S) this map equals $\xi_j$ on $\I$, by
...
\begin{lemma}[Regular variation of the projected quantile]
\label{lem:app-quantile-rv}
If $\bar F(y)=\Pp(Y>y\mid U_j=u)$ is regularly varying at infinity with
index $-1/\xi$, $\xi>0$, then its generalized upper quantile
$Q(s)=\bar F^{-1}(1/s)$ is regularly varying with index $\xi$, i.e.\
$Q(s)=s^{\xi}\ell(s)$ with $\ell$ slowly varying.
\end{lemma}

\begin{proof}
This is the standard inversion property of regularly varying functions
\citep[Proposition~B.1.9(9)]{deHaanFerreira2006}: the generalized
inverse of a regularly varying nonincreasing function with index
$-1/\xi$ is regularly varying with index $\xi$ at the appropriate
argument, and the representation follows from the definition of slow
variation.
\end{proof}
...
$\alpha m_n^-\ge2$ (since $\lfloor x\rfloor\ge x/2$ for $x\ge2$).  For
the upper bound, $m_n^-\le2h(n+1)-1\le2hn+2h-1\le2hn+1$, so
$K_n\le\alpha m_n^-\le2n\alpha h+1$.
\end{proof}
...
\begin{proof}
This lemma uses only the fibre identity \eqref{eq:fibre}, proved in
Appendix~\ref{app:proofs} without reference to this subsection, through
its consequence that inactive coordinates have $\Psi_j=\gmax$
(Proposition~\ref{prop:geometry}, claim 1); no circularity arises.
Hence

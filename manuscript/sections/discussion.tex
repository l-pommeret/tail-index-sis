...
$\Delta_{\min}\gg1/\log n$. Appendix~\ref{app:examples} shows that the
severity of this restriction is governed by the conditional mass at the
fibre maximum. Third, rare favourable configurations: even with a
comfortable population gap, the envelope near its maximum is estimable only
from observations whose remaining active coordinates are simultaneously
near their maximizing values, and the mass of such configurations can decay
exponentially in $s$. Model M4 in Section~\ref{sec:simulation} shows the
resulting cost.
...
Sections~\ref{sec:simulation} and~\ref{sec:realdata}: inspect stability of
the leading set over a modest $(a,b)$ grid and under half-sampling, retain
a generous $d$, and reserve interpretation for variables that survive these
perturbations. Dependence deserves the same caution: it can help, by
concentrating active configurations, or hurt, by promoting inactive
neighbours, and the direction is design-specific.
